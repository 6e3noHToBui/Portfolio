...
exports.addMessage = [
    ...
    check('username', "Username is incorrect").trim().isLength({ min: 4, max: 20 }),
    check('message', "Message is incorrect").trim().escape().notEmpty().isLength({max:500}), async (req, res) => {
        const { username, email, message } = req.body;

        try {
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
                const error = errors.errors.map(err => err.msg);
